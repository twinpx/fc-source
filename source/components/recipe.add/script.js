...
$(document).ready(function() {
  ...
  $("#save_recipe").click(function() {
	...
	$("#dish_stages").delegate("div.ingredient a.delete_icon", "click", function() {
		...
		$(this).siblings("div.search_list").children("ul").css({display:"none"}).empty();
	}).delegate("span.scales", "mouseenter", function() {
		$(this).children(".hint").css({visibility:"visible"});
	}).delegate("span.scales", "mouseleave", function() {
		$(this).children(".hint").css({visibility:"hidden"});
	}).delegate("span.scales", "click", function() {
...
function addStage() {
	...
	var newStage = $('<div class="stage"><div class="delete_icon"><a href="#" title="Удалить этап"></a></div><h2>' + numberingStageHeader(newStageNumber) + ' этап</h2><div class="row"><div class="col-sm-6 description"><div class="form-group"><h5>Описание ' + numberingStage(newStageNumber) + ' этапа<span class="no_text">?</span></h5><textarea name="stage_description[]" cols="10" rows="10" class="form-control"></textarea></div><div class="form-group"><h5>Фото этапа (600х400 px)</h5><div class="input_file"><div class="blocker"></div><input type="file" name="photo[]" class="text customFile"><div class="browse_button" title="Выбрать файл"><input type="button" value="Обзор"></div><div class="new_file_name"></div></div></div></div><div class="col-sm-6 ingredient"><h5>Ингредиенты ' + numberingStage(newStageNumber) + ' этапа<span class="scales"><span class="hint"><span>Таблица мер</span></span></span></h5><div class="stage_ing_list"></div><div class="add_ingredient"><span class="icon"></span><a href="#">Добавить ингредиент</a></div></div></div><div class="clear"></div></div>');
...
function checkStageForm() {
	...
	$("#add_recipe_form span.no_text").each(function() {
		$(this).closest(".form-group").find("input.form-control:visible").each(function() {
			if(this.value == "") {
				if($(this).attr("type") == "file") {
					if($(this).closest("div.form-group").children("div.file_name").size() == 0) {
						sendStageFormFlag = 1;
						$(this).closest("div.form-group").addClass("attention");
			...
			else {$(this).closest("div.form-group").removeClass("attention");
			}
		});
		...
		$(this).closest("div.form-group").find("select").each(function() {
		...
		$(this).closest("div.form-group").find("textarea").each(function() {
